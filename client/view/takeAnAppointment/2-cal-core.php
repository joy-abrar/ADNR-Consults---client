...
class Calendar 
{
  ...
  function save ($start, $end, $txt, $color, $id=null) 
  {
    ...
    if ($id==null) 
    {
      $sql = "INSERT INTO `events` (`evt_start`, `evt_end`, `evt_text`, `evt_color`, `clientId`, `praticienId`) VALUES (?,?,?,?,?,?)";
      $data = [$start, $end, $txt, $color, $userId, $userId];
    } 
    else 
    {
  ...
  function get ($month, $year) 
  {
    ...
    // (F2) GET EVENTS
    if (!$this->exec(
      "SELECT * FROM `events` WHERE ( (`evt_start` BETWEEN ? AND ?) OR (`evt_end` BETWEEN ? AND ?) OR (`evt_start` <= ? AND `evt_end` >= ?)) AND (`clientId` = ?)", 
